...
[![dshfind](https://dshfind.com/api/badge/PerryLink/dsh-observe?metric=downloads&lang=es)](https://dshfind.com/es/plugins/PerryLink/dsh-observe?ref=badge)
...
- **npm 0.1.7-rc.1** — el plugin se desarrolla y prueba contra `@deepseek-ai/dsh@0.1.7-rc.1` (devDeps y la regla primaria de CI); el rango de peers `>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0 || >=0.1.6-0 <0.2.0 || >=0.1.7-0 <0.2.0` mantiene instalable cada línea publicada, y la segunda regla (`typecheck:ci`) más el workflow compat cubren los baselines antiguos.
...
pnpm run typecheck  # tsc: src + tests contra los devDeps 0.1.7-rc.1 (sin tsconfig paths)
