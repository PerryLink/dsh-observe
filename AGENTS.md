...
- `src/collector.ts` — the event→span/metric collector over a real `Session`; spans open/close at boundary events, missing closers close with error status, retries derive from identical (name, arguments) pairs, and prompt/completion bodies come only from the logged header and the session surface (model-visible ⟺ logged). The LLM span finish reason and first-token timing read both host generations: the legacy `assistant/chunk` events (published rc.1 line) and the v2 `assistant/message` embedded `stream` (0.1.3-alpha.1, read structurally through local minimal types because that line is not published to npm).
...
- `typecheck` (`tsconfig.json` + `tsconfig.test.json`) and `typecheck:ci` (`tsconfig.ci.json`, `skipLibCheck: false` + `verbatimModuleSyntax`) both resolve `@deepseek-ai/*` through `node_modules` to the pinned `0.1.2-rc.1` devDeps — no tsconfig `paths` to a harness checkout is configured, so the two rulers check the same published types. The package ships against rc.1.
