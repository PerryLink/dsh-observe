...
- `test/` — vitest; REAL `Context`/`SessionStore`/`Session` and the REAL storage seam (dsh-storage + dsh-storage-json backend in a per-test temp dir + dsh-storage-domain facility) from the 0.1.0-rc.6 peers. Only the network edge (global `fetch`) is scripted. Message-producing session events must carry their `surfaceOp` intent when appended (rc.6 contract).
...
- `typecheck` resolves `@deepseek-ai/*` through tsconfig paths to the local harness checkout; `typecheck:ci` clears the paths and checks against the published `0.1.0-rc.6` types. Both must stay green — the package ships against rc.6.
