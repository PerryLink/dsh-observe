...
| 方面 | 状态 |
|---|---|
| Harness | DeepSeek Harness `dsh-v0.1.5-rc.2`（2026-09-09 已适配）：会话格式 V3 把助手流内嵌到 `assistant/message` / `assistant/attempt`，系统提示词成为 surface 节点 0（`system/message`）；本插件只消费实时事件流，从不读取会话日志文件。已于 2026-09-11 对照 dsh-v0.1.5-rc.2 tag 核验（本地完整门禁链；profile 安装冒烟由月度 compat 工作流覆盖）。 |
| Node | `^22.19.0 \|\| >=24.0.0` |
...
- **npm 0.1.5-rc.2** —— 插件针对 `@deepseek-ai/dsh@0.1.5-rc.2` 开发与测试（devDeps 与 CI 均钉此版本）；peer 复合范围 `>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0` 保持已发布的 rc.1 线可安装，更新的 harness 基线由月度 compat 工作流验证。
...
pnpm run typecheck  # tsc：src + tests，对照钉定的 0.1.5-rc.2 devDeps（无 tsconfig paths）
