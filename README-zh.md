...
| 方面 | 状态 |
|---|---|
| Harness | DeepSeek Harness `dsh-v0.1.6-alpha.2`（2026-09-18 已适配）：会话格式 V3 把助手流内嵌到 `assistant/message` / `assistant/attempt`，系统提示词成为 surface 节点 0（`system/message`）；本插件只消费实时事件流，从不读取会话日志文件。peer 复合范围 `>=0.1.2-rc.1 <0.2.0 \|\| >=0.1.5-alpha.1 <0.2.0 \|\| >=0.1.6-0 <0.2.0` 保持每条已发布线可安装（本地完整门禁链；profile 安装冒烟由 compat 工作流覆盖）。 |
| Node | `^22.19.0 \|\| >=24.0.0` |
...
- **npm 0.1.6-alpha.2** —— 插件针对 `@deepseek-ai/dsh@0.1.6-alpha.2` 开发与测试（devDeps 与 CI 主尺子）；peer 复合范围 `>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0 || >=0.1.6-0 <0.2.0` 保持每条已发布线可安装，旧基线与第二把尺子（`typecheck:ci`）及 compat 工作流共同覆盖。
...
pnpm run typecheck  # tsc：src + tests，对照 0.1.6-alpha.2 devDeps（无 tsconfig paths）
