#!/usr/bin/env node
/**
 * Release automation for dsh-observe.
 *
 ...
 *      verify:self-contained, verify:artifacts, readme-sync) against the
 *      bumped version — the version-consistency tripwire (test/version.spec.ts)
 *      runs inside.
 *   4. Commit `chore(release): <version>` and create the annotated `v<version>`
 *      tag (message: `dsh-observe v<version>`).
 *
 ...
 *
 * @module dsh-observe/scripts/release
 */
...
// The OTLP instrumentation scope carries the version as a hardcoded string
// (test/version.spec.ts trips when it drifts from package.json) — keep it
...
git('tag', '-a', `v${version}`, '-m', `dsh-observe v${version}`)
